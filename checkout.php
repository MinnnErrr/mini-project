...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['order_id'])) {
    ...
    try {
        ...
        if ($payment_method === 'MembershipCard') {
            ...
            if (!$membership || $membership['Balance'] < $total_price) {
                throw new Exception("Insufficient balance in Membership Card.");
            }

            // Deduct balance and add points
            $new_balance = $membership['Balance'] - $total_price;

            $stmt = $conn->prepare("UPDATE membershipcard 
            ...
            $stmt = $conn->prepare("INSERT INTO point (Date, Amount, MembershipID) VALUES (NOW(), :points, :membership_id)");
            $stmt->bindParam(':points', $points, PDO::PARAM_INT);
            $stmt->bindParam(':membership_id', $membership['MembershipID'], PDO::PARAM_INT);
            $stmt->execute();
        }
