...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("
        SELECT p.BasePrice, pp.Price 
        FROM printingpackage p 
        JOIN packageproperty pp ON p.PackageID = pp.PackageID 
        WHERE p.PackageID = ? AND pp.PropertyID = ?
    ");
    ...
    $stmt = $conn->prepare("
        INSERT INTO `order` 
        (TotalPrice, Status, Points, PickUpDate, PickUpTime, CustomerID, file, descriptionOrder) 
        VALUES (?, 'Pending', ?, ?, ?, ?, ?, ?)
    ");
    ...
    $stmt = $conn->prepare("
        INSERT INTO orderprintingpackage (Quantity, OrderID, PackageID) 
        VALUES (?, ?, ?)
    ");
